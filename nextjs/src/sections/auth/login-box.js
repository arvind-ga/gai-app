        ...
        try {
            ...
            if (error.response) {
                if (error.response.status === 401) {
                    ...
                    toast.error(`Service temporarily unavailable. Please try again later. Error: ${error}`);
                } else {
                    // Handle other errors
                ...
                toast.error(`The server is not responding. Please check your internet connection or try again later. Error: ${error}|| ${error.response}`);
            } else {
                // Something happened in setting up the request that triggered an Error
                toast.error(`An error occurred during login. Please try again. Error: ${error}|| ${error.response}`);
            }
        }
    };
            ...
            >
                ...
                <Box component="form" noValidate sx={{ mt: 3, mb: 2, width: '100%' }} onSubmit={handleLogin}>
                    <TextField
                        ...
                        InputProps={{ style: { color: '#333' } }} // Light text color
                    />
                    <TextField
                        margin="normal"
                        required
